...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $notlp = isset($_POST['notlp']) ? $_POST['notlp'] : '';
        $pekerjaan = isset($_POST['pekerjaan']) ? $_POST['pekerjaan'] : '';
        
        // Update the record
        $stmt = $pdo->prepare('UPDATE contact SET id = ?, nama = ?, email = ?, notlp = ?, pekerjaan = ? WHERE id = ?');
        $stmt->execute([$id, $nama, $email, $notlp, $pekerjaan, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contact['id']?>" method="post">
        ...
        <input type="text" name="notlp" value="<?=$contact['notlp']?>" id="notlp">
